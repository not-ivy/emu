...
export default function Tabs() {
  ...
  return (
    <div className="text-purple-400 font-bold text-2xl flex flex-col items-end">
      <button aria-label='home' className={`${selected === 'home' ? 'text-purple-200' : ''} flex items-center`} onClick={() => {setSelectedPanel('home')}}>
        {selected === 'home' ? <HiHome className="w-10 h-10 m-2 md:w-8 md:h-8" /> : <HiOutlineHome className='w-10 h-10 m-2 md:w-8 md:h-8' />}
        <span className="hidden md:block">Home</span>
      </button>
      <button aria-label='messages' className={`${selected === 'messages' ? 'text-purple-200' : ''} flex items-center`} onClick={() => {setSelectedPanel('messages')}}>
      ...
      <button aria-label='profile' className={`${selected === 'profile' ? 'text-purple-200' : ''} flex items-center`} onClick={() => {setSelectedPanel('profile')}}>
      ...
      <button aria-label='more' className={`${selected === 'more' ? 'text-purple-200' : ''} flex items-center`} onClick={() => {setSelectedPanel('more')}}>
